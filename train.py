    ...
    with torch.autocast(device_type=device.type, dtype=torch.float16, enabled=True):
        ...
        for timestep in sampling_timesteps:
            ...
            if is_teacher:
                ...
                pred = model(x=model_input, semantics=semantics, timestep=timestep, timestep_2=timestep)
            latent = sampler.step(pred=pred, x_t=latent, t=timestep)
    
    return latent + 0.5

@torch.no_grad()
def infer_model_meanflow(
    args,
    model,
    semantic_encoder,
    image,
    ...
    train_timesteps = torch.linspace(args.T, 0, args.num_train_timesteps + 1).round().int()
    idx = torch.linspace(0, len(train_timesteps) - 1, sampling_steps + 1).int()

    sampling_timesteps = train_timesteps[idx].to(device)
    timesteps = sampling_timesteps[:-1]
    timesteps_next = sampling_timesteps[1:]
    ...
    with torch.autocast(device_type=device.type, dtype=torch.float16, enabled=True):
        ...
        for t, r in zip(timesteps, timesteps_next):
            _t, _r = t / args.T, r / args.T
            model_input = torch.cat([latent, cond], dim=1)
            pred = model(x=model_input, semantics=semantics, timestep=r, timestep_2=t)
            latent = latent - (_t - _r) * pred
...
#                               Timestep sampling                               #
#################################################################################
def sample_timesteps(timesteps, batch_size, device, ratio_r_equal_t=0.5):
    # 1) sample two timesteps within the training timesteps
    idx = torch.rand(batch_size, len(timesteps)).argsort(dim=1)[:, :2]      # argsort() -> for batched implementation
    sampled_timesteps = timesteps[idx]

    # 2) Ensure t > r by sorting
    sorted_timesteps,  _ = torch.sort(sampled_timesteps, dim=1, descending=True)
    t, r = sorted_timesteps[:, 0], sorted_timesteps[:, 1]

    # 3) control the portion of r = t samples
    equal_mask = torch.rand(batch_size) < ratio_r_equal_t
    r = torch.where(equal_mask, t, r)

    return t.to(device), r.to(device)
    
def sample_three_timesteps(timesteps, batch_size, device, ratio_r_equal_t=0.5):
    # 1) sample three timesteps within the trainig timesteps
    idx = torch.rand(batch_size, len(timesteps)).argsort(dim=1)[:, :3]      # argsort() -> for batched implementation
    sampled_timesteps = timesteps[idx]

    # 2) Ensure t > s > r by sorting
    sorted_timesteps,  _ = torch.sort(sampled_timesteps, dim=1, descending=True)
    t, s, r = sorted_timesteps[:, 0], sorted_timesteps[:, 1], sorted_timesteps[:, 2]
    lam = (t - s) / (t - r)

    # # 3) control the portion of r = t samples
    equal_mask = torch.rand(batch_size) < ratio_r_equal_t
    s = torch.where(equal_mask, t, s)
    r = torch.where(equal_mask, t, r)

    return t.to(device), s.to(device), r.to(device), lam.to(device)

#################################################################################
#                                 Visualization                                 #
#################################################################################
@torch.no_grad()
def visualize(args, model, semantics_encoder, vis_loaders, save_dir, device, sampling_steps=1, pred_only=True):
    cmap = matplotlib.colormaps.get_cmap('Spectral')
...
def main(args):
    ...
    # 1) Teacher model: instantaneous velocity prediction model
    teacher_dit_block_kwargs = {
        "fused_attn": True,
    }
    if args.teacher:
        teacher_model = DiT(time_embedding='single', **teacher_dit_block_kwargs)
        teacher_model.load_state_dict(_checkpoint, strict=False)
        teacher_model = teacher_model.to(device)
        requires_grad(teacher_model, False)
    else:
        teacher_model = None
    
    # 2) Student model: average velocity prediction model
    dit_block_kwargs = {
        "fused_attn": True,
    }
    model = DiT(time_embedding='dual_v1', **dit_block_kwargs)
    ...
    if accelerator.is_main_process:
        logger.info(f"Dataset contains {len(concat_dataset):,} images ({args.data_dir})")
    ...
    train_timesteps = torch.linspace(1000, 0, num_train_timesteps + 1).round().int() # '+1' for 0
    # train_timesteps = train_timesteps.float() # cast to float32 for jvp
    ...
    for epoch in range(num_train_epochs):
        for batch in train_dataloader:
            ...
            with accelerator.accumulate(model):
                ...
                depth_gt = batch[gt_depth_type].to(device)      # ~ [-1, 1], torch.tensor([b, 1, h, w])
                ...
                # # sample timesteps (t >= r)
                # t, r = sample_timesteps(
                #     timesteps=train_timesteps,
                #     batch_size=batch_size,
                #     device=device,
                #     ratio_r_equal_t=args.ratio_r_equal_t,
                # )

                # sample timesteps (t >= r)
                t, s, r, lam = sample_three_timesteps(
                    timesteps=train_timesteps,
                    batch_size=batch_size,
                    device=device,
                    ratio_r_equal_t=args.ratio_r_equal_t,
                )
                _t, _s, _r = (t / T).view(-1, 1, 1, 1), (s / T).view(-1, 1, 1, 1), (r / T).view(-1, 1, 1, 1)
                lam = lam.view(-1, 1, 1 ,1)

                # #jvp NOTE: jvp gives OOM issue even if with batch_size 2... cannot train effeciently...
                # def u_func(model_input, r, t):
                #     return model(x=model_input, semantics=semantics, timestep=r, timestep_2=t)
                
                # with torch.no_grad():
                #     # _, dudt = torch.func.jvp(u_func, (model_input, r, t), (v_t.repeat(1, 4, 1, 1), torch.zeros_like(r), torch.ones_like(t)))
                #     _, dudt = torch.autograd.functional.jvp(u_func, (model_input.detach(), r, t), (v_t.repeat(1, 4, 1, 1).detach(), torch.zeros_like(r), torch.ones_like(t)))
                #     u_target = v_t - (_t - _r) * dudt


                # #### SplitMeanFlow training! #####
                # sample noise
                noise = torch.randn_like(depth_gt)

                # prepare model input
                x_t = (1 - _t) * depth_gt + _t * noise
                model_input = torch.cat([x_t, cond], dim=1)


                # model predicted average velocity
                u_pred = model(x=model_input, semantics=semantics, timestep=r, timestep_2=t)

                with torch.no_grad():
                    # Forward pass 1: u(z_t, s, t), flow from t -> s
                    u2 = model(x=model_input, semantics=semantics, timestep=s, timestep_2=t)
                    x_s = x_t - (_t - _s) * u2

                    model_input_x_s = torch.cat([x_s, cond], dim=1)
                    # Forward pass 2: u(x_s, r, s), flow from s -> r
                    u1 = model(x=model_input_x_s, semantics=semantics, timestep=r, timestep_2=s)
                
                u_target = (1 - lam) * u1 + lam * u2

                if teacher_model is not None:
                    with torch.no_grad():
                        v_t = teacher_model(x=model_input, semantics=semantics, timestep=t)
                else:
                    v_t = depth_gt - noise

                # mask out model_pred to r == t batches to predict v_t 
                mask_r_equal_t = r == t
                u_target = torch.where(mask_r_equal_t.view(-1, 1, 1, 1), v_t, u_target)
                
                # loss
                # error = u_pred - u_target
                # loss = torch.sum((error ** 2).reshape(error.shape[0], -1), dim=-1)
                # if args.weighting == "adaptive":
                #     weights = 1.0 / (loss.detach() + 1e-3).pow(args.adaptive_p)
                #     loss = weights * loss

                # loss_mean_ref = torch.mean((error ** 2))
                if valid_depth_mask is not None:
                    u_pred = u_pred[valid_depth_mask]
                    u_target = u_target[valid_depth_mask]
                
                loss = loss_func(u_pred.float(), u_target.float())
            ...
            if accelerator.is_main_process and global_step % args.logging_steps == 0:
                ...
                logger.info(f"Step {global_step}: loss = {logs['loss']:.4f}, loss_scaled(x100) = {logs['loss_scaled']:.4f}")

...
# V1: weighted sum of two sinusoidal timestep embeddings
class DualTimestepEmbedderV1(nn.Module):
    """
    Embeds two scalar timesteps into vector representations.
    """
    ...
    def forward(self, r, t):
        r_freq = self.timestep_embedding(r, self.frequency_embedding_size)
        t_freq = self.timestep_embedding(t, self.frequency_embedding_size)
        
        # weighted sum of two sinusoidal timestep embeddings.
        # NOTE: this does not hurt the original weights when r == t.
        freq = 0.5 * r_freq + 0.5 * t_freq
...
class DiT(nn.Module):
    ...
    def __init__(
        ...
        num_heads=16,
        mlp_ratio=4.0,
        time_embedding='single', # ['single', 'dual_v1', 'dual_v2']
        **block_kwargs,
    ):
        super().__init__()
        ...
        if time_embedding == 'single':
            self.t_embedder = TimestepEmbedder(hidden_size)
        elif time_embedding == 'dual_v1':
            self.t_embedder = DualTimestepEmbedderV1(hidden_size)
        else:
            raise NotImplementedError

        self.blocks = nn.ModuleList(
            [DiTBlock(hidden_size, num_heads, mlp_ratio=mlp_ratio, rope=self.rope, **block_kwargs) for _ in range(depth)]
        )
    ...
    def forward(self, x=None, semantics=None, timestep=None, timestep_2=None, dropout=0.1):
        """
        ...
        if timestep_2 is not None and len(timestep_2.shape) == 0:
            timestep_2 = timestep_2[None]
        ...
        if timestep_2 is None and isinstance(self.t_embedder, TimestepEmbedder):
            t = self.t_embedder(timestep)
        elif timestep_2 is not None and isinstance(self.t_embedder, DualTimestepEmbedderV1):
            t = self.t_embedder(timestep, timestep_2)  # (N, D)
        else:
            raise ValueError

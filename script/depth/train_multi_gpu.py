...
if "__main__" == __name__:
    ...
    assert accumulation_steps.is_integer(), "accumulation steps must be integer"
    accumulation_steps = int(accumulation_steps)

    accelerator_project_config = ProjectConfiguration(
        project_dir=output_dir,
        logging_dir="logging_accelerator",   
    )
    ...
    try:
        AcceleratorState().deepspeed_plugin.deepspeed_config['train_batch_size_per_gpu'] = batch_size_per_gpu
    ...
    # -------------------- Device --------------------
    logging.info(f"RANK[{accelerator.process_index}] device = {accelerator.device}")
    ...
    if accelerator.is_main_process:
        logging.info(
            ...
            f'Train with deepspeed: {train_with_deepspeed}'
        )
    ...
    if accelerator.is_main_process: logging.debug("Augmentation: ", cfg.augmentation)
